...
res = re.search("__FILE__\), '', '", spc_functions)

prodVersions = re.findall("(?<=.js: )[0-9]+\.[0-9]+", prod_versions)
prodFilenames = re.findall("[a-zA-Z_0-9]+(?=.js: )", prod_versions)

filenames = re.findall("(?<=plugins_url\('/js/)[a-zA-Z_0-9]+(?=.js)", spc_functions)
versions = re.findall("(?<=__FILE__\), '', ')[0-9]+\.+[0-9]+", spc_functions)
...
#update versions
for cv in currentVs:
    updated_spc_functions = re.sub(f"(?<=plugins_url\('/js/{cv.filename}.js', __FILE__\), '', ')[0-9]+\.+[0-9]+", f"{cv.v}", updated_spc_functions)
