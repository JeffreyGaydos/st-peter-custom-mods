...
extractName = re.search(f"(?<=.includes.css.)spc_.*(?=\.css)", file)
if extractName == None:
...
for js in jsToTick:
    ...
    spc_version = re.search(f"(?<=plugins_url\('/js/{jsv.filename}.js', __FILE__\), '', ')[0-9]+\.+[0-9]+", spc_functions)
    if spc_version == None:
        pprint(f"{err}Could not find file {jsv.filename}")
        stop(False)

    jsv.v = spc_version.group()
    ...
    updated_spc_functions = re.sub(f"(?<=plugins_url\('/js/{jsv.filename}.js', __FILE__\), '', ')[0-9]+\.+[0-9]+", f"{jsv.v}", updated_spc_functions)
